...
export class TreeSift {
   ...
   */
  static analyze(filePath: string, test: boolean = false): ComponentContext {
    ...
    if (test) {
      try {
        // Try to read from the fixture file
        const fixturePath = path.join(process.cwd(), 'src/test/fixtures/SampleComponent.tsx');
        if (fs.existsSync(fixturePath)) {
          code = fs.readFileSync(fixturePath, 'utf8');
        } else {
          // Fallback to hardcoded string if file doesn't exist
          code = `
        ...
        console.error('Error reading test fixture:', error);
        throw new Error('Failed to analyze test component');
      }
    } else {
